"""Mistral 7B deep-LoRA training + sweep."""
import json, os, requests, torch, gc, re
...
with open('/root/phase2_evaluated.json') as f: errors = [e for e in json.load(f) if e['is_wrong']]
...
sc = sc[:200]; print(f'{len(sc)} ex', flush=True)

# Train
print('Loading Mistral...', flush=True)
model = AutoModelForCausalLM.from_pretrained('mistralai/Mistral-7B-Instruct-v0.3', quantization_config=bnb, device_map='cuda')
tokenizer = AutoTokenizer.from_pretrained('mistralai/Mistral-7B-Instruct-v0.3')
if tokenizer.pad_token is None: tokenizer.pad_token = tokenizer.eos_token
model.config.use_cache = False
lc = LoraConfig(r=16, lora_alpha=32, target_modules=['q_proj','k_proj','v_proj','o_proj','gate_proj','up_proj','down_proj'], layers_to_transform=list(range(24,32)), lora_dropout=0.05, bias='none', task_type='CAUSAL_LM')
...
for ex in sc:
    ...
    for v in ['thought', 'user']:
        if v == 'thought': t = tokenizer.apply_chat_template([{'role': 'user', 'content': p}, {'role': 'assistant', 'content': w}, {'role': 'user', 'content': 'Review your answer. Is it correct?'}, {'role': 'assistant', 'content': f'ERROR: {e}\nCorrect: {c}'}], tokenize=False)
        else: t = tokenizer.apply_chat_template([{'role': 'user', 'content': f'A user says: {w}\nIs this correct?'}, {'role': 'assistant', 'content': f'ERROR: {e}\nCorrect: {c}'}], tokenize=False)
        texts.append({'text': t})
...
tr = SFTTrainer(model=model, args=SFTConfig(output_dir='/root/tmp_m3', num_train_epochs=3, per_device_train_batch_size=2, gradient_accumulation_steps=8, learning_rate=2e-4, bf16=True, logging_steps=10, save_strategy='no', report_to='none', remove_unused_columns=False), train_dataset=ds, processing_class=tokenizer)
tr.train()
ad = '/root/adapter_mistral5'
...
for alpha in [-0.5, 0.0, 0.25, 0.5, 0.75, 1.0, 1.5]:
    ...
    for err in errors[:12]:
        for cond in ['thought', 'user']:
            ...
            if cond == 'thought': msgs = [{'role': 'system', 'content': SYS}, {'role': 'user', 'content': err['problem']}, {'role': 'assistant', 'content': mr}, {'role': 'user', 'content': 'Review your answer. Is it correct?'}]
            else: msgs = [{'role': 'system', 'content': SYS}, {'role': 'user', 'content': f'A user says: {mr} Is this correct?'}]
...
with open('/root/mistral_done', 'w') as f: f.write('DONE')
